<template>
  <div class='home-category' @mouseleave="categoryId=null">
    <ul class="menu">
      <li :class="{active:categoryId===item.id}" v-for="item in menuList" v-bind:key="item.id" @mouseenter="categoryId=item.id">
        <RouterLink v-bind:to="`/category/${item.id}`">{{ item.name }}</RouterLink>
        <template v-if="item.children">
          <RouterLink v-for="sub in item.children" :key="sub.id" v-bind:to="`/category/sub/${sub.id}`">{{ sub.name }}</RouterLink>
