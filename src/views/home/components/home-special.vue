<template>
  <HomePanel title="最新专题">
    ...
    <div class="special-list" ref="target">
      <div class="special-item" v-for="special in specialList" :key="special.id">
        <RouterLink to="/">
          <img v-lazy="special.cover" alt />
          <div class="meta">
            <p class="title">
              <span class="top ellipsis">{{special.title}}</span>
              <span class="sub ellipsis">{{special.summary}}</span>
            </p>
            <span class="price">&yen;{{special.lowestPrice}}</span>
          </div>
        </RouterLink>
        <div class="foot">
...
export default {
  ...
  setup () {
    const { target, result } = useLazyData(findSpecial) // 1. 请求到懒加载的专题数据

    return { target, specialList: result } // 把数据变量返回给模板层使用
  }
}
</script>
